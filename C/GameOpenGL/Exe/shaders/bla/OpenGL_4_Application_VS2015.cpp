...
int main()
{
	...
	Model cottageOBJ("objects/cottage/Snow covered CottageOBJ.obj");
	...
	glm::vec3 swordScale = glm::vec3(0.08f, 0.08f, 0.08f);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glm::mat4 character;
		character = glm::mat4();
		character = glm::translate(character, glm::vec3(0.0f, -2.0f, 0.0f));
		if (attack == true && abs(camera.Position.x - character[0][0]) < 1.5f && abs(camera.Position.z - character[1][0]) < 2.0f && abs(camera.Position.z - character[2][0]) < 1.5f)
		{
			dead = true;
		}

		if(dead==true)
			character = glm::rotate(character, glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f));

		character = glm::scale(character, glm::vec3(1.2f, 1.2f, 1.2f));
		modelShader.setMat4("model", character);
		modelOBJ.Draw(modelShader);
		...
		int t=glfwGetTime();
		if(t%10==0)
			model = glm::rotate(model, -(float)glfwGetTime()*5.1f, glm::vec3(0.0f, 0.0f, 1.0f));
		modelShader.setMat4("model", model);
		dragonOBJ.Draw(modelShader);

		model = glm::mat4();
		model = glm::translate(model, glm::vec3(5.0f, -3.35f, -5.0f));
		...
		//HAMMER
		//-----------------------------------------------------------------
		if (abs(camera.Position.x - hammerPos.x) < 0.5 && abs(camera.Position.y - hammerPos.y) < 0.5 && abs(camera.Position.z - hammerPos.z) < 0.5)
			closetohammer = true;
		...
		if (attack == true && ((hammer == true && sword == false) || selectHammer == true))
		{
			...
			model = glm::rotate(model, (float)(-attackAngle * 5), glm::vec3(1.0f, 0.0f, 0.0f));
			model = glm::translate(model, glm::vec3(0.1f, 0.3f, -0.2f));
		...
		//SWORD
		//-----------------------------------------------------------------
		if (abs(camera.Position.x - swordPos.x) < 0.5 && abs(camera.Position.y - swordPos.y) < 0.5 && abs(camera.Position.z - swordPos.z) < 0.5)
			closetosword = true;
...
void processInput(GLFWwindow *window)
{
	...
	if (glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS && closetosword==true || (glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS && glfwGetKey(window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS))
	{
		sword = true;
	}
	if (glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS && closetohammer == true || (glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS && glfwGetKey(window, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS))
	{
		hammer = true;
	}
