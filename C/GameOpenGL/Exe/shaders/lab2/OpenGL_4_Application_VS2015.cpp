...
int main()
{
	...
	Shader depthMapShader("shaders/simpleDepthMap.vert", "shaders/simpleDepthMap.frag");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		nanosuitOBJ.Draw(depthMapShader);
		...
		modelShader.use();
		modelShader.setMat4("lightSpaceTrMatrix", lightSpaceTrMatrix);
		glm::mat4 view;
		float radius = 10.0f;
		...
		glBindTexture(GL_TEXTURE_2D, depthMapTexture);
		glUniform1i(glGetUniformLocation(modelShader.ID, "shadowMap"), 3);
