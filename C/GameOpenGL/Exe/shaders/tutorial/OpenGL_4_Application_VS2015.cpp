...
int main()
{
	...
	Shader simpleDepthShader("shaders/pointshadowdepth.vert", "shaders/pointshadowdepth.frag", "shaders/pointshadowdepth.gs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		{ 
			...
			simpleDepthShader.setVec3("lightPos", pointLightPositions[i]);
			simpleDepthShader.use();
			...
			nanosuitOBJ.Draw(simpleDepthShader);

			//GROUND
			model = glm::mat4();
			model = glm::translate(model, glm::vec3(0.0f, -2.0f, 0.0f));
			modelShader.setMat4("model", model);
			groundOBJ.Draw(simpleDepthShader);
